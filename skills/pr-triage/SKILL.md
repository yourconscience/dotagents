...
Prefer the configured GitHub MCP server for PR/check/thread data when the agent can access MCP tools. Use the local `pr-triage` tool as the deterministic fallback and as the hook runtime path:
...
The local tool currently uses `gh` as its fallback backend because shell hooks cannot directly call the host's in-process MCP tools.
